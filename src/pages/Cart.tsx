...
const {data}=useGetAllProductsQuery({});
  return (
   <div className="min-h-screen  mx-auto lg:gap-36 flex flex-col lg:flex-row my-16 px-6 justify-between items-center lg:items-start ">
    ...
    <div>
      {
        data?.data?.map((product:IProduct,idx:number)=><div key={idx} className="py-10 border-b flex justify-between lg:bg-slate-50 px-8 gap-3 mb-3">
      ...
      <button className="w-10 text-3xl active:scale-95 bg-gray-100 flex justify-center items-center"><span>+</span></button>
            </div>
         ...
         <p className="font-semibold ml-8">{product.name}</p>
          ...
          <div className="flex flex-col justify-between items-end">
            <span  className="hover:scale-110 text-xl active:scale-95"><RxCross2 /></span>
            <span className="font-bold"><span className="text-3xl font-bold">&#2547;</span>3847</span>
          </div>
